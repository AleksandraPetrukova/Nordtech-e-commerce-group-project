...
const AllProductPage = ({ darkMode, addToCart }) => {
  ...
  // by default, all the items will be displayed
  useEffect(() => {
    fetch(`/get-items`)
      .then((res) => res.json())
      .then((data) => {
  ...
  // gets all items from the category or body location
  // selected from the drop downs
  useEffect(() => {
    fetch(`/get-${category}`)
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <>
      ...
      <StyledSelect>
        ...
        {/*  Body Location selections Below */}
        <select onClick={handleChange}>
